...
static u8* DTD(u64 cur_ms, u64 event_ms) {
  ...
  t_s = (delta / 1000) % 60;
	...
	if (initial_fuzzing == 0){
		temperature = pow(20.0, - ((double) delta / (double) (totalTimeout / 2)));
	}
	if ((initial_fuzzing == 2) && !direct_start && delta >= (totalTimeout/10)){
		direct_start = 1;
	}
	if (r_h < t_h) r_h = t_h;
	if (direct_start && (t_h == r_h)){
...
void record_relevance(int t){
  FILE* f;
	u8 * fn = alloc_printf("%s/initial_relevance.%d.txt", out_dir, t);
	s32 fd;
	fd = open(fn, O_WRONLY | O_CREAT| O_EXCL, 0600);
	ck_free(fn);
	...
	//record score of each test case - cheong
	/*
	fprintf(f, "testcase score\n");
	struct queue_entry * q4 = queue;
	while(q4){
		fprintf(f, "%s, %lf\n", q4->fname, q4->relscore);
		q4 = q4->next;
	}
	*/
	close(fd);
...
void initial_fuzz(char ** argv){
	...
	close(fd);
	stage_short = "init";
	stage_name = "initial_fuzz";
	out_buf = ck_alloc_nozero(len);
	memcpy(out_buf, orig_in, len);
	stage_max = len;

	for (stage_cur = 0; stage_cur < stage_max ; stage_cur ++){
		u8 init_v = out_buf[stage_cur];
		for (byte_v = 0; byte_v < 256; byte_v ++){
			out_buf[stage_cur] = byte_v;
