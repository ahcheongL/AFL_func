...
bool AFLCoverage::runOnModule(Module &M) {
  ...
  int inst_blocks = 0;
	...
	for (auto &F : M){
		...
		if (bb_exist) func_idx++;
	}
	unsigned int num_func = func_idx;
	func_idx = 0;

	std::ifstream finfocount;
	finfocount.open("FICount.txt", std::ifstream::in);
	unsigned int num_funcinfo = 0;
	char tmpstr[100];
	memset(tmpstr,0,sizeof(tmpstr));
	finfocount.getline(tmpstr,sizeof(tmpstr));
	num_funcinfo = std::atoi(tmpstr);
	finfocount.close();

	std::ofstream finfocountout;
	finfocountout.open("FICount.txt", std::ofstream::out | std::ofstream::trunc);
	finfocountout << (num_funcinfo + 1) << "\n";
	finfocountout.close();

	std::ofstream funcinfos;
	memset(tmpstr,0,sizeof(tmpstr));
  sprintf(tmpstr,"FInfo%d.txt", num_funcinfo);
  ...
  for (auto &F : M){
      ...
      inst_blocks++;
		...
		if (numBlocks) funcinfos << F.getName().str() << " " << numBlocks << "\n";
